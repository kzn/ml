...
public class MulticlassCS {
	...
	public MulticlassCS(Dataset ds, int[] targets, double c, double eps, int maxiter, Kernel k){
	   ...
	   */
	  /*void add_alphas(int vec_idx, pair<int, double> *delta, int delta_size){
	    for(const feat_t *it = ds->vec_begin(vec_idx); it != ds->vec_end(vec_idx); it++){
	      for(int i = 0; i != delta_size; i++)
		w_add(delta[i].first, it->first,delta[i].second*it->second);
	    }
	  }*/
	  ...
	  public void solve(){
	    ...
	    //int *delta_idx = new int[ds->size()];
	    //double delta_val = new double[ds->size()];
	    //pair<int, double> *delta = new pair<int, double>[ds->size()];
	    int delta_size = 0;
